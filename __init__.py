...
from ballsdex.packages.wheel.cog import Wheel
...
def command_count(cog: Wheel) -> int:
    total = 0
...
def strip_descriptions(cog: Wheel):
    for command in cog.walk_app_commands():
        command.description = "."
        if isinstance(command, app_commands.Group):
            continue
...
async def setup(bot: "BallsDexBot"):
    ...
    if command_count(n) > 3900:
        ...
        log.warn("/wheel command too long, stripping descriptions.")
    await bot.add_cog(n)
